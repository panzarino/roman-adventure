...
<body>
    ...
    <header id="top" class="header" style="background: url(img/farmer.jpg) no-repeat center center scroll;-webkit-background-size: cover;-moz-background-size: cover;background-size: cover;-o-background-size: cover;">
        <div class="text-vertical-center">
            ...
            <?php
            $name = $_GET['name'];
                echo "<p style='font-size:20px'><mark>Now that you are creating the emperor's wine, you think about how unjust the emperor is.<br>Do you want to poison the wine?</mark></p>";
                echo '<form class="form-inline" action="poison.php" method="get">';
                echo '<input type="hidden" name="name" value="'.$name.'">';
                echo '<button type="submit" class="btn btn-default">Yes (Poison)</button></form><br>';
                echo '<form class="form-inline" action="emperorservantgrapes.php" method="get">';
                echo '<input type="hidden" name="name" value="'.$name.'">';
                echo '<button type="submit" class="btn btn-default">No</button></form>';
            ?>
            <br>
        </div>
    </header>
